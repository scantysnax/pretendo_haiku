...
// -----------------------------------------------------------------------------
// capture_scope_sample
//
// Captures one debugger oscilloscope sample containing the instantaneous output
// level of each APU channel together with the already-generated mixed PCM
// sample.
//
// The mixed sample is supplied by the normal APU audio-generation path rather
// than generated here.  This is important because mix_channels() contains a
// stateful DC-blocking filter and must only be called once for each output
// sample.
//
// Parameters:
//   mixedSample - Final mixed unsigned 8-bit PCM sample generated by the APU.
//
// Returns:
//   Nothing.
// -----------------------------------------------------------------------------
...
capture_scope_sample(uint8_t mixedSample)
{
	...
	sample.noise = noise.debug_output();
	sample.dmc = dmc.output();

	sample.mixed = static_cast<float>(mixedSample);
...
// -----------------------------------------------------------------------------
// nes::apu::debug_scope_snapshot
//
// Copies the current APU oscilloscope sample history into caller-provided
// storage.
//
// Samples are returned in chronological order from oldest to newest.  If the
// caller supplies less storage than the number of available samples, the most
// recent samples are returned.
//
// Parameters:
//   samples  - Destination sample array.
//   capacity - Maximum number of samples that may be copied.
//
// Returns:
	...
	uint32_t capacity)
{
	...
	if (available == APU_SCOPE_SAMPLE_CAPACITY) {
		start = scope_sample_next_;
	}
...
// tick
//
// Advances the APU by one APU cycle.
//
// This handles frame-counter timing, IRQ generation, audio sample generation,
// debugger oscilloscope capture, and channel clocking.
//
// The mixed audio sample is generated exactly once per host output sample.  The
// debugger scope captures that already-generated sample so the stateful
// DC-blocking filter inside mix_channels() is not advanced more than once.
//
// Parameters:
//   None.
//
...
tick()
{
	...
	if ((apu_cycles_ % ClocksPerSample) == 0) {
		...
		if (debug_audio_is_muted()) {
			...
			sample_buffer_[sample_buffer_end] = mixedSample;
		}
...
apu_write_log_snapshot(apu_write_log_entry_t *entries, uint32_t capacity)
{
	...
	for (uint32_t index = 0; index < count; index++) {
		const uint32_t physicalIndex = (start + index) % APU_WRITE_LOG_CAPACITY;
